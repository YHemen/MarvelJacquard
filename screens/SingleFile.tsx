...
useEffect(() => {
  ...
  const getFile = async (fileName) => {
    ...
    if (!fileName) {
      Alert.alert('Please enter valid file names with extensions (e.g., design.bmp)');
      return;
    }
    
    try {
      if(fileName ){
        const bodyImageUrl = `file:///storage/emulated/0/Android/media/com.whatsapp/WhatsApp/Media/WhatsApp Documents/${fileName}`;
        setImageUri(bodyImageUrl);
...
const styles = StyleSheet.create({
  ...
  closeButton: {
    ...
    right: 20, // Adjust for horizontal spacing
    width: 50,
    height: 50,
