...
const AppDrawer = ({isConnected}) => {
  ...
  const divertToWeb = () => {
    const url = 'https://marveljacquard.com/admin/'; // Replace with your target URL https://marveljacquard.com/admin/
    ...
    >
      <Drawer.Screen 
        ...
        options={{
          headerLeft: () => (
            isConnected ? (
              <TouchableOpacity onPress={divertToWeb}>
                 <Text style={{marginLeft:10}}><Icon name="copy" color="white" size={20} /></Text>
                 <Text style={{color: 'white', marginLeft:5}}>Merge</Text>
              </TouchableOpacity>
            ) : null
          ),
          drawerIcon: ({ color, size }) => (<Icon name="spinner" color={color} size={size} />),
