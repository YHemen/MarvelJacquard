...
import React, {
  ...
  export const MyContextProvider: React.FC<{children: ReactNode}> = ({ children }) => {
    ...
    const [bleNotifyValue, setBleNotifyValue] = useState<'1' | '2' | null>(null);
    ...
    const [designDir, setDesignDir] = useState<string>('1'); // '1' = left, '2' = right
    const [lockStatus, setLockStatus] = useState<'1' | '2'>('1');
    // const [lockStatus, setLockStatus] = useState('');
    // const [lockStatus, setLockStatus] = useState<'locked' | 'unlocked'>('unlocked');
    ...
    const readAllCharDataFromEvent = (data: any) => {
      ...
      if (service === serviceid2 && characteristic === caracid8) {
        console.log('Raw BLE value (Base64):', value);
        const caracid8Value = bytesToString(value);
        console.log('caracid8 Data recieved from Esp32:', caracid8Value);
        ...
        if (caracid8Value.startsWith('LR/')) {
          const lrnotify = caracid8Value.substring(3).replace(/^\/+/, '');
          console.log('caracid Left Right Status PChar22:', lrnotify);
          setDesignDir(lrnotify);
          console.log('Machine Left Right Status from pchar22', lrnotify);
        }
        if (caracid8Value.startsWith('DS/')) {
          const dsnotify = caracid8Value.substring(3).replace(/^\/+/, '');
          console.log('caracid Running File Status PChar22:', dsnotify);
          if (dsnotify === '1' || dsnotify === '2') {
            setBleNotifyValue(dsnotify);
          }
          console.log('Single Double Status from pchar22', dsnotify);
        }
        if (caracid8Value.startsWith('LS/')) {
          const lsnotify = caracid8Value.substring(3).replace(/^\/+/, '');
          console.log('caracid Lock Status:', lsnotify);
          setLockStatus(lsnotify);
          console.log('Machine Lock Status', lockStatus);
        }
  ...
  BleManager.getDiscoveredPeripherals().then((result: any) => {
    if (result.length === 0) {
      ...
      const filteredDevices = result.filter((item: any) =>
        item.name && (item.name.startsWith('MJ') || item.name.startsWith('Marvel'))
      );

      setDevice(filteredDevices);
      console.log('Filtered Devices from HandleConnection', filteredDevices);

      filteredDevices.forEach(async (bleDevice) => {
        console.log('Local Name of the Device:', bleDevice.advertising?.localName);
        setLocalNamed(bleDevice.advertising?.localName);
        ...
        // ✅ Auto-connect if it matches previously saved device ID
        if (bleDevice.id === lastDeviceId) {
          console.log('✅ Auto-connecting to previously known device:', bleDevice.name);
          await onConnect(bleDevice);
        }
      });
    }
...
useEffect(() => {
  const checkBluetoothAndScan = async () => {
    ...
    if (isEnabled) {
      requestPermission(); // Will trigger startScanning -> handleGetConnectedDevices
    }
  };
  ...
  console.log('🧭 isConnected changed to:', isConnected);
    ...
    const renderItem = ({item}: any) => {
      ...
      <View style={[styles.bleCard, currentDevice?.id === item?.id && isConnected && styles.connectedCard]}>
  ...
  {/* <View style={styles.bleCard}> */}
    ...
    const onConnect = async (item: any) => {
      try {
        ...
        // ✅ Request larger MTU
        await BleManager.requestMTU(item.id, 25); // or 247 if your ESP32 supports it
        console.log('✅ MTU request sent (25 bytes)');
        setCurrentDevice(item);
        setDeviceId(item.id);
        setIsConnected(true);
        console.log('Connection Status is:', isConnected);
        await AsyncStorage.setItem('lastConnectedDeviceId', item.id); // 👈 Save it here
  ...
  const onDisconnect = async (item: any) => {
  try {
    await BleManager.disconnect(currentDevice?.id);
    // await AsyncStorage.removeItem('lastConnectedDeviceId'); // 👈 Remove it
    setCurrentDevice(null);
    setIsConnected(false);
    clearInterval(item.id);
  } catch (error) {
  ...
  // const bytesToString = (bytes: ArrayBuffer | number[] | Uint8Array): string => {
  ...
  // const bytesToString = (base64Value: string): string => {
  ...
  if (!isConnected || !deviceId || !serviceid1 || !caracid2) {
    console.warn('BLE not ready. Skipping write.');
  ...
  try {
    const testRun = 'FT/';
    const notifyRun = stringToBytes(testRun); // Ensure this returns correct byte format
    console.log('🟡 Writing FT/ to trigger ESP32...');
    await BleManager.write(deviceId, serviceid1, caracid2, notifyRun);
    const results = bytesToString(notifyRun); // Optional debug log
    console.log('✅ FT/ sent successfully', results);
  } catch (error) {
  ...
  const lockMachine = async () => {
    if (isConnected) {
      // const str = "CALL_FUNCTION";
      const lockme = stringToBytes("MU/2");
      await BleManager.write(deviceId, serviceid1, caracid2, lockme)
      .then(() => {
        const results = bytesToString(lockme);
        console.log('Function call command sent',results);
        setLockStatus(results);
        })
        .catch((error) => {
  ...
  const runningFile = async (runmode: '1' | '2') => {
    if (!isConnected) {
    ...
    try {
      const prefix = 'SD/';
      const command = prefix + runmode;
  
      console.log('Command to send:', command);
  
      const commandBytes = stringToBytes(command);
  
      await BleManager.write(deviceId, serviceid1, caracid2, commandBytes);
  
      const sentString = bytesToString(commandBytes);
      console.log('Function call command sent:', sentString);
  
      setBleNotifyValue(runmode); // ✅ Only update if write succeeded
    } catch (error) {
      console.error('Error sending function call command:', error);
...
// Define fetchData once at the top level inside your component
const fetchData = async (localNamed: string) => {
  setLoading(true);
  try {
    console.log('Fetching data with localName:', localNamed);

    const response = await axios.get('https://marveljacquard.com/apis.php', {
      params: { localName: localNamed },
    });
...
// Run fetchData whenever localNamed changes
// useEffect(() => {
//   if (localNamed) {
//     fetchData(localNamed);
//   }
// }, [localNamed]);
  ...
  const styles = StyleSheet.create({
    ...
    connectedCard: {
  backgroundColor: '#fa9f1e', // emerald green to indicate connected alternative color is #B6E696 #82B842 #D73C54 #8EC9BC #DB5746 #00C853 #FF5733 #6F8FAF #C7395F #E26274 #B2456E 
},
    btn: {
      width: 100,
      height: 40,
